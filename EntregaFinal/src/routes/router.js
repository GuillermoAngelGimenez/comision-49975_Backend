...
export class MiRouter{
    ...
    get(ruta, permisos, ...funciones){ // ... son aquí el operador rest, que alamacena los argumentos que lleguen en un array llamado funciones
        this.router.get(ruta, this.misRespuestas, (ruta!=="/api/sessions/current" && ruta!=="/github" &&  ruta!=="/callbackGithub" && ruta!=="/github" &&  ruta!=="/callbackGithub" && ruta!=="/recupero02")?passportCall("jwt"):(req,res,next)=>{next()}, this.acceso(permisos), this.agregaTryCatch(funciones))        
    }
    ...
    post(ruta, permisos, ...funciones){ // ... son aquí el operador rest, que alamacena los argumentos que lleguen en un array llamado funciones
        this.router.post(ruta, this.misRespuestas, (ruta!=="/login" && ruta!=="/registro" && ruta!=="/recupero" &&  ruta!=="/recupero03")?passportCall("jwt"):(req,res,next)=>{next()}, this.acceso(permisos), this.agregaTryCatch(funciones))
    }
